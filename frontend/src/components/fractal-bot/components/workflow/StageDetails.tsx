...
export default function StageDetails({ stage, onStepClick }: StageDetailsProps) {
    ...
    const handleAddStep = () => {
        ...
        const newStep: Step = {
            ...
            inputs: [],
            outputs: [],
            tool: stepType === 'tool' ? { name: state.currentMission.selectedTools.find(t => t.id === selectedTool)?.name || '', configuration: {} } : undefined,
                    ...
                    {stage.steps && stage.steps.length > 0 ? (
                        stage.steps.map((step) => (
                            ...
                            >
                                ...
                                {step.tool && (
                                    <span className="mt-2 inline-block px-2 py-1 text-xs font-medium text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 rounded">
                                        {step.tool.name}
                                    </span>
                                )}
                            </div>
